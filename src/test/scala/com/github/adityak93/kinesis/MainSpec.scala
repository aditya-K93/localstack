...
class MainSpec extends CatsEffectSuite with ScalaCheckEffectSuite {
  ...
  class TestKinesisClient(
    ...
    def getRecords(shardIterator: String): IO[ProcessingResult] = {
      ...
      if (count > errorAfter) IO.raiseError(new Exception("Simulated failure"))
      else if (records.isEmpty) IO.pure(ProcessingResult(List(), "", None))
      else {
  ...
  def setupTestEnv: Resource[IO, (Ref[IO, RunningStats], Channel[IO, String])] = for {
    ...
    Files[IO].tempFile.use { tempFile =>
      setupTestEnv.use { case (stats, errorChannel) =>
        ...
        val client = new TestKinesisClient(
          ...
          override def getRecords(shardIterator: String): IO[ProcessingResult] = {
            if (shardIterator.contains("shard-1")) {
        ...
        for {
          fiber <- KinesisApp
            ...
            .concurrently(KinesisApp.writeSums(tempFile.toString, stats, defaultConfig.emitFrequency))
            .take(2)
            .compile
            .drain
            .start
  ...
  test("should handle retry policy with exponential backoff") {
    ...
    val client = new TestKinesisClient {
      override def getRecords(shardIterator: String): IO[ProcessingResult] = {
        attemptCount += 1
        if (attemptCount <= 2) IO.raiseError(new Exception("Temporary failure"))
  ...
  test("should log final give up message after max retries") {
    val logger = org.typelevel.log4cats.slf4j.Slf4jLogger.getLogger[IO]
