...
object KinesisApp:
    ...
    object KinesisClient:
        ...
        private def logError[F[_]](err: Throwable, details: RetryDetails)(
            using L: Logger[F]
        ): F[Unit] =
                        ...
                        shards <- F.fromCompletableFuture(
                          F.delay(
                            client
                                ...
                                .thenApply(
                                  _.streamDescription().shards().asScala.map(_.shardId()).toList
                                )
                          )
                        )
                                    ...
                                    L.warn(
                                      s"Sequence number no longer valid, resetting to LATEST"
                                        ...
                                        tryGetIterator(
                                          ...
                                          shardId,
                                          None
                                        ) // Try again with no sequence number
                                case e =>
                                    F.raiseError(
                                      e
                                    ) // Let other errors be handled by retry policy
                            }
                            .retryingOnErrors(
                              policy = retryPolicy,
    ...
    def kinesisStream[F[_]: {Logger, Temporal}](
        ...
        def makeProcessShard(shardId: String): Stream[F, Unit] = {
            def loop(
                ...
                Stream
                    ...
                    .flatMap {
                        case ProcessingResult(events, nextIterator, newLastSeqNum) =>
                            Stream.eval {
            ...
            Stream
                ...
                .handleErrorWith { error =>
                    Stream.eval(
                      errorChannel.send(error.getMessage) *>
                          Logger[F].error(s"Fatal error in shard $shardId: ${error.getMessage}")
                    ) *>
        ...
        Stream
            ...
            .flatMap { shardIds =>
                Stream.emits(shardIds).map(makeProcessShard).parJoin(config.maxConcurrency)
            }
            .handleErrorWith { error =>
                Stream.eval(
    ...
    def writeSums[F[_]: {Async, Files, Logger}](
        ...
        Stream.eval(Files[F].exists(path)).flatMap { exists =>
            ...
            headerStream ++
                Stream.repeatEval(stats.get).metered(emitEvery.seconds).evalMap {
                    currentStats =>
                        val timestamp =
